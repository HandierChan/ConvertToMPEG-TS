...
def openConfigFile(openGUIRead=False):
    if openGUIRead==False:
        ...
        hoconConfig=pyhoconConfig.openHoconFile(selectPathFileExt)
    else:
...
def quitWindow():
    try:pyhoconConfig.saveHoconFile(pyhoconConfig.createAppDataPath(softwareName,'presets')+'/history.txt',allConfigParam(writeIOData=True))
    except:pass
    tk.quit()
    tk.destroy()
    exit()
...
ffmpegPathButton = Button(IOFrame, text='Select',command=lambda:ffmpegConvert.selectFFmpegPath(ffmpegPathVar))
...
inputVideosButton = Button(IOFrame, text='Select',command=lambda:ffmpegConvert.selectVideoFiles(inputVideosVar))
...
outputPathButton = Button(IOFrame,text='Select',command=lambda:ffmpegConvert.selectOutputPath(outputPathVar))
...
def executeParamCmd():
    # 调用 ffmpegConvert.py 命令输出，export=1输出|0打印
    commandCmd = commandText.get('1.0', "end")
    ffmpegConvert.outputAllVideos(programPathNameExt=ffmpegPathEntry.get(),importFiles=inputVideosEntry.get(),userCommand=commandCmd,exportPath=outputPathEntry.get(),exportExt=containerFormat.get(),export=1)
